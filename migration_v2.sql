...
--
-- Versão idempotente: pode ser rodada várias vezes sem erro.
-- ═══════════════════════════════════════════════════════════════
...
DROP POLICY IF EXISTS "advogada_assistente_full" ON pessoas;
CREATE POLICY "advogada_assistente_full" ON pessoas
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON processos;
CREATE POLICY "advogada_assistente_full" ON processos
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON partes_processo;
CREATE POLICY "advogada_assistente_full" ON partes_processo
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON honorarios;
CREATE POLICY "advogada_assistente_full" ON honorarios
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON parceiros_processo;
CREATE POLICY "advogada_assistente_full" ON parceiros_processo
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON lancamentos;
CREATE POLICY "advogada_assistente_full" ON lancamentos
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON movimentacoes;
CREATE POLICY "advogada_assistente_full" ON movimentacoes
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON compromissos;
CREATE POLICY "advogada_assistente_full" ON compromissos
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON tarefas;
CREATE POLICY "advogada_assistente_full" ON tarefas
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON comentarios;
CREATE POLICY "advogada_assistente_full" ON comentarios
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON atendimentos;
CREATE POLICY "advogada_assistente_full" ON atendimentos
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON audit_log;
CREATE POLICY "advogada_assistente_full" ON audit_log
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

DROP POLICY IF EXISTS "advogada_assistente_full" ON perfis;
CREATE POLICY "advogada_assistente_full" ON perfis
  FOR ALL USING (perfil_atual() IN ('advogada','assistente'));

-- ── Estagiário: tudo EXCETO financeiro global ──

DROP POLICY IF EXISTS "estagiario_pessoas" ON pessoas;
CREATE POLICY "estagiario_pessoas" ON pessoas
  FOR ALL USING (perfil_atual() = 'estagiario');

DROP POLICY IF EXISTS "estagiario_processos" ON processos;
CREATE POLICY "estagiario_processos" ON processos
  FOR ALL USING (perfil_atual() = 'estagiario');

DROP POLICY IF EXISTS "estagiario_partes" ON partes_processo;
CREATE POLICY "estagiario_partes" ON partes_processo
  FOR ALL USING (perfil_atual() = 'estagiario');

DROP POLICY IF EXISTS "estagiario_movimentacoes" ON movimentacoes;
CREATE POLICY "estagiario_movimentacoes" ON movimentacoes
  FOR ALL USING (perfil_atual() = 'estagiario');

DROP POLICY IF EXISTS "estagiario_compromissos" ON compromissos;
CREATE POLICY "estagiario_compromissos" ON compromissos
  FOR ALL USING (perfil_atual() = 'estagiario');

DROP POLICY IF EXISTS "estagiario_tarefas" ON tarefas;
CREATE POLICY "estagiario_tarefas" ON tarefas
  FOR ALL USING (perfil_atual() = 'estagiario');

DROP POLICY IF EXISTS "estagiario_comentarios" ON comentarios;
CREATE POLICY "estagiario_comentarios" ON comentarios
  FOR ALL USING (perfil_atual() = 'estagiario');

DROP POLICY IF EXISTS "estagiario_atendimentos" ON atendimentos;
...
DROP POLICY IF EXISTS "financeiro_lancamentos" ON lancamentos;
CREATE POLICY "financeiro_lancamentos" ON lancamentos
  FOR ALL USING (perfil_atual() = 'financeiro');

DROP POLICY IF EXISTS "financeiro_honorarios_read" ON honorarios;
CREATE POLICY "financeiro_honorarios_read" ON honorarios
  FOR SELECT USING (perfil_atual() = 'financeiro');

DROP POLICY IF EXISTS "financeiro_processos_read" ON processos;
CREATE POLICY "financeiro_processos_read" ON processos
  FOR SELECT USING (perfil_atual() = 'financeiro');

DROP POLICY IF EXISTS "financeiro_pessoas_read" ON pessoas;
CREATE POLICY "financeiro_pessoas_read" ON pessoas
  FOR SELECT USING (perfil_atual() = 'financeiro');

DROP POLICY IF EXISTS "financeiro_perfis" ON perfis;
...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    'pessoas','processos','partes_processo','lancamentos',
    'movimentacoes','compromissos','tarefas','comentarios',
    'atendimentos','honorarios','parceiros_processo'
  ]
  LOOP
    BEGIN
      EXECUTE format('ALTER PUBLICATION supabase_realtime ADD TABLE %I', t);
    EXCEPTION WHEN duplicate_object THEN
      -- tabela já está na publicação, seguir adiante
      NULL;
    END;
  END LOOP;
END $$;
